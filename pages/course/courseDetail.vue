<template>
	<view>
		...
		<view class="bottomBar">
			<button class="service" id='btn_col'>客服</button>
			<button v-if="userInfo.nickName" class="collection" id='btn_col'>收藏</button>
			<button v-else open-type="getUserInfo" data-index="collection" @getuserinfo="bindGetUserInfo" id='btn_col' class="collection">收藏</button>
			<view v-if="userInfo.nickName" @click="goSignup(CourseDetail.courseId)" class="sign-up">立即报名</view>
			<button v-else open-type="getUserInfo" data-index="signup" @getuserinfo="bindGetUserInfo" class="sign-up">立即报名</button>
		</view>
	</view>
</template>
			...
			bindGetUserInfo(e) {
				...
				if (e.mp.detail.userInfo) {
					getApp().getUserInfo().then(res => {
						that.userInfo = res
						// 跳转到对应页面
						if (e.currentTarget.dataset.index === 'signup') {
							that.goSignup(that.CourseDetail.courseId)
						} else if ('collection') {
							// todo  点击收藏的事件
						}
					}).catch(e => {})
